...
export async function PATCH(
  ...
  if (clearStockObjetivo && isAlmacenArea(existing.area)) {
    ...
    const roundStockNullable = (value: number | null) =>
      value == null ? null : normalizeNivelStock(value);

    await upsertStockObjetivo(
      cn,
      normalizeNivelStock(Number((objUpdate.stockMinimo as number | undefined) ?? current?.stockMinimo ?? 0)),
      normalizeNivelStock(Number((objUpdate.puntoPedido as number | undefined) ?? current?.puntoPedido ?? 0)),
      roundStockNullable(
