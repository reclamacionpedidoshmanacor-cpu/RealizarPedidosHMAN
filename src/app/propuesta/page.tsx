...
export default function PropuestaPage() {
  ...
  const handleDeshacer = async () => {
    if (!data) return;
    if (!confirm('¿Deshacer la tramitación? La propuesta volverá a estado borrador y el recuento quedará pendiente de nuevo.')) return;
    setDeshaciendo(true);
    try {
            ...
            {data.propuesta.estado === 'tramitada' && (
              <>
                <button
                  ...
                  title="Revertir a borrador para editar y volver a tramitar"
                >
                ...
                {data.lineas.map((linea, idx) => {
                  ...
                  return (
                    <tr key={linea.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      ...
                      {/* Stock objetivo: Min y Máx sin PP */}
                      ...
                      {/* Stock actual */}
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block rounded-md px-3 py-1 text-base font-semibold tabular-nums ${
                          bajoPP
