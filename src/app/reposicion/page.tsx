...
export default function ReposicionPage() {
  ...
  const detalleAgrupado = useMemo(() => {
    const destinos = new Map<string, Map<string, Linea[]>>();
    for (const linea of detalle?.lineas ?? []) {
      const origen = linea.ubicacionOrigen ?? 'Sin ubicación asignada';
      if (!destinos.has(linea.ubicacion)) destinos.set(linea.ubicacion, new Map());
      const grupos = destinos.get(linea.ubicacion)!;
      if (!grupos.has(origen)) grupos.set(origen, []);
      grupos.get(origen)!.push(linea);
    }
    return [...destinos]
      .sort((a, b) => a[0].localeCompare(b[0], 'es'))
      .map(([destino, grupos]) => [
        destino,
        [...grupos].sort((a, b) => a[0].localeCompare(b[0], 'es')),
      ] as [string, [string, Linea[]][]]);
  }, [detalle]);

  const totalDetalle = useMemo(
    () => (detalle?.lineas ?? []).reduce((suma, linea) => suma + (cantidades[linea.id] ?? linea.cantidadCajas), 0),
    [detalle, cantidades],
  );
  ...
  return (
    <div className="space-y-5">
      ...
      {detalle && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-slate-900/40 p-4">
          <div className="my-8 flex max-h-[88vh] w-full max-w-4xl flex-col rounded-2xl bg-white shadow-xl">
              ...
              {detalleAgrupado.map(([ubicacionDestino, grupos]) => (
                <section key={ubicacionDestino} className="overflow-hidden rounded-xl border border-slate-200">
                  <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-2.5">
                    <h3 className="font-semibold text-slate-800">{ubicacionDestino}</h3>
                    <span className="text-xs text-slate-500">Ubicación destino</span>
                  </div>
                  {grupos.map(([ubicacionOrigen, filas]) => (
                    <div key={ubicacionOrigen}>
                      <p className="border-b border-slate-100 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
                        Origen: {ubicacionOrigen}
                      </p>
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-slate-100 text-left text-[11px] uppercase tracking-wide text-slate-400">
                            <th className="w-[90px] px-4 py-2">CN</th>
                            <th className="px-3 py-2">Marca</th>
                            <th className="w-[130px] px-4 py-2 text-right">Cantidad</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {filas.map((linea) => (
                            <tr key={linea.id}>
                              <td className="px-4 py-2 font-mono text-xs text-slate-500">{linea.codigo}</td>
                              <td className="px-3 py-2 text-slate-800">
                                {linea.nombre}
                                {linea.notas && <span className="block text-[11px] text-amber-700">Nota: {linea.notas}</span>}
                              </td>
                              <td className="px-4 py-2">
                                <div className="flex items-center justify-end gap-2">
                                  <input
                                    ...
                                    aria-label={`Cantidad en ${linea.unidadPedido} de ${linea.nombre}`}
                                  />
                                  <span className="w-12 text-left text-xs text-slate-500">{linea.unidadPedido === 'unidades' ? 'ud.' : 'cajas'}</span>
                                </div>
                              </td>
                            </tr>
                          ))}
